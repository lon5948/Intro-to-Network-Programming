...
void Status(int newClient, vector<string> recVecTCP) {
    string sendBack = "";
    int server[3];
    string line = "";

    if (recVecTCP.size() != 1) {
        sendBack = "Usage: status\n";
    }
    else {
        ifstream myfile ("./efs/status.txt");
        int ind = 0;
        if (myfile.is_open()) {
            while (getline(myfile,line)) {
                server[ind] = stoi(line);
                ind++;
            }
            myfile.close();
            sendBack += "Server1: " + to_string(server[0]) + "\n";
            sendBack += "Server2: " + to_string(server[1]) + "\n";
            sendBack += "Server3: " + to_string(server[2]) + "\n";
            int len = sendBack.length();
            char sendMessage[len] = {};
            sendBack.copy(sendMessage, len);
...
void Login(int newClient, vector<string> recVecTCP) {
    ...
    if (recVecTCP.size() != 3) {
        ...
        if (it == nameMap.end()) {
            ...
            server[2] += 1;
            ofstream outfile("./efs/status.txt");
            if (outfile.is_open()) {
                outfile << to_string(server[0]) << "\n";
                outfile << to_string(server[1]) << "\n";
                outfile << to_string(server[2]) << "\n";
                outfile.close();
            }
            else cout << "Unable to open file" << endl;
        }
...
void* Connection(void* data) {
    ...
    while (1) {
        ...
        else if (recVecTCP[0] == "status") {
            Status(newClient, recVecTCP);
        }
        else {
...
int main(int argc, char* argv[]) {  
    ...
    ofstream newfile("./efs/status.txt");
    if (newfile.is_open()) {
        newfile << "0\n0\n0\n";
        newfile.close();
    }
